...
where TKey : notnull
{
   ...
   // readonly HandlingSubscribeDisposedPolicy handlingSubscribeDisposedPolicy;
    ...
    public MessageBrokerCore()
        //(MessagePipeDiagnosticsInfo diagnotics, MessagePipeOptions options)
    {
    ...
    sealed class HandlerHolder : IDisposable
        //, IHandlerHolderMarker
    {
        ...
        public void Dispose()
        {
            ...
            {
                //handlers(FreeList)
                //countの変換とbool判定でdiagnoticsの監視を操作しているので
                //diagnoticsに併せてオミットして必要分のDispose処理に切り替え
                /*
