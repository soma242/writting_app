...
namespace PublishStructure
{
    public static partial class SubscriberExtensions
    {
        ...
        //this ISubscriber<TMessage> subscriberの部分で拡張メソッドとして定義している
        //実際に取る引数はAction<TMessage> handler(とparams)
        public static IDisposable Subscribe<TMessage>(this ISubscriber<TMessage> subscriber, Action<TMessage> handler)
            //, params MessageHandlerFilter<TMessage>[] filters)　引数をオミット
        {
            //MessageBrokerのCoreにAnonymousMessageHandler<TMessage>を渡している。
